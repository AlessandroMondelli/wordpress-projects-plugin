...
class AmProjectLayoutMenu extends AmProjectMenus
{
    ...
    {
        parent::add_options_submenu('edit.php?post_type=am_projects', 'Layout Progetti', 'Layout Progetti', 'manage_options', 'layout_progetti', [$this, 'add_layout_submenu'], 4);
    }
    ...
    public function layout_setting_section()
    {
        ...
        //Layout
        parent::setting_field('layout_ch', 'Scegli layout', [$this, 'choose_layout'], 'layout_progetti', 'layout_section', ['name' => 'layout', 'id' => 'layout', 'value' => get_option('layout'), 'options' => ['grid' => 'Griglia', 'no-grid' => 'No Griglia']]);
        parent::register_fields('layout_group', 'layout', []);

        //Height
        if (get_option('layout') === 'grid') {
            parent::setting_field('layout_img_height', 'Altezza immagini', [$this, 'get_img_height'], 'layout_progetti', 'layout_section', ['name' => 'img_height', 'id' => 'img_height', 'value' => get_option('img_height')]);
            parent::register_fields('layout_group', 'img_height', []);
        }
        ...
        //Attivazione filtri
        parent::setting_field('layout_filters', 'Attiva filtri', [$this, 'toggle_filters'], 'layout_progetti', 'layout_section', ['name' => 'filters_activated', 'id' => 'filters_activated', 'value' => get_option('filters_activated')]);
        parent::register_fields('layout_group', 'filters_activated', []);
    ...
    //Funzione scelta layout
    public function choose_layout($args)
    {
...
?>
        <select name="<?php echo $name ?>" id="<?php echo $id ?>">
            ...
            ?>
                <option <?php selected($value, $key) ?> value="<?php echo $key ?>"><?php echo $option ?></option>
            <?php
            }
            ?>
        </select>
    ...
    //Funzione altezza immagini
    public function get_img_height($args)
    {
        $name = (isset($args['name'])) ? $args['name'] : '';
        $id = (isset($args['id'])) ? $args['id'] : '';
        $value = (isset($args['value'])) ? $args['value'] : '';
    ...
    ?>
        <input name="<?php echo $name ?>" id="<?php echo $id ?>" type="number" min="10" max="1000" value="<?php echo $value ?>">
    <?php
    }
    ...
    //Funzione attivazione filtri
    public function toggle_filters($args)
    {
        $name = (isset($args['name'])) ? $args['name'] : '';
        $id = (isset($args['id'])) ? $args['id'] : '';
        $active = (isset($args['value'])) ? $args['value'] : 0;

        echo '<input name="' . $name . '" id="' . $id . '" type="checkbox" value="1" ' . checked(1, (int)$active, false) . '>';
    }
    ...
    ?>
        ...
        <img class="amprojects-admin-img" src="<?php echo ADMIN_PLUGIN_URL . '/src/media/projects-colors.jpg' ?>" style="height: 200px; object-fit:contain;">
    <?php
    }
